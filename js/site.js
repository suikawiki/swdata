...
Object.defineProperty (SWD.Char.prototype, 'cid', {
  get: function () {
    if (this.type === 'char') {
      var m = this._char.match (/^:(?:ac|ag|aj|ak|aj2-|ak1-|aj-ext-)([0-9]+)$/);
      if (m) return m[1];
    }
...
Object.defineProperty (SWD.Char.prototype, 'cidFontKey', {
  get: function () {
    if (this.type === 'char') {
      var m = this._char.match (/^:(ac|ag|aj|ak|ak1-|aj-ext-)(?:[0-9]+)$/);
      if (m) return {
        'aj-ext-': '霧明朝',
      }[m[1]] || m[1];
    }
...
Object.defineProperty (SWD.Char.prototype, 'hasImageSet', {
  get: function () {
    return !! this.char.match (/^:wakan-[\u3042-\u3093][0-9]+$/);
  },
}); // char.hasImageSet

SWD.Char.prototype.getImageSetInfo = function () {
  var m = this.char.match (/^:(wakan)-([\u3042-\u3093][0-9]+)$/);
  return SWD.Font.load ({name: m[1]}).then (async (font) => {
    await font._load ();
...
defs (() => {
  ...
  def.pcHandler = function (templates, obj) {
    if (obj.type === 'unicode' || obj.type === 'vs') {
      ...
      } else if (/^:(?:gw-.|mh?[0-9]|b5-cdp-|extf|irg2017-|kx|kps|sat|gtk?[0-9]|koseki[0-9]|touki[0-9]|jistype-|u-nom-|u-uk[ab]-|u-gbdot|u-dakuten|UK-|gb[0-9]|ks0-|omron|mule|iscii|tis|viscii|isolatin|isogreek|isocyrillic|isoarabic|isohebrew|koi|wingreek|dsf|wakan)/.test (char)) {
...
SWD.Char.RelData._getGlyphInfo = hasWorkerMethod ('SWDCharRelDataGetGlyphInfo', async function (opts) {
  ...
  if (opts.mapper === 'auto') {
    ...
    if (m = char.match (/^:(mh?[0-9]|b5-cdp-|irg2017-|extf|kx|kps|sat|gtk?[0-9]|koseki[0-9]|touki[0-9]|UK-)/)) {
      ...
      if (m1) {
        char = m1.char
        if (m[1] !== 'b5-cdp-') approximate = true;
      } else {
        return null;
      }
    }
  ...
  props: {
    ...
    swUpdate: async function () {
      ...
      var insertImageSet = async (c, schar) => {
        ...
        var ts = await getISTemplate;
        is.keys.forEach (key => {
          var li = ts.createFromTemplate ('li', {
            fontName: is.name,
            glyphName: key,
            char: ':' + is.name + '-' + key,
          });
  ...
  props: {
    ...
    swCreateRouteElement: function (route, {partial, activeChar}) {
      ...
      route.forEach (step => {
        ...
        c.setAttribute ('template', 'sw-data-char-item-in-rel-ext');
  ...
  props: {
    ...
    swUpdate: async function () {
      ...
      if (char.type === 'unicode' || char.type === 'vs' ||
        ...
        var m = char.char.match (/^:(a[jcgk]|ak1-|aj-ext-)([1-9][0-9]*|0)$/);
        if (m) {
          var glyphId = m[2];
          var ts = await $getTemplateSet ('sw-char-fonts-font-item');
          var key = m[1];
          var names = [];
          if (key !== 'aj-ext-') names.push (key);
          ...
          if (key === 'aj' || key === 'aj-ext-') names = ['霧明朝'];
          for (var i = 0; i < names.length; i++) {
            var name = names[i];
            var info = await SWD.Font.info ({name});
            if (info.hasAJ1GlyphNames) {
